...
public class SpawnerREIClientPlugin implements REIClientPlugin {
    ...
    public void registerDisplays(DisplayRegistry registry) {
        List<SpawnerModifier> recipes = new ArrayList<>(Minecraft.getInstance().level.getRecipeManager().getAllRecipesFor(SpawnerModifier.TYPE));
        recipes.sort((r1, r2) -> r1.getOffhandInput() == Ingredient.EMPTY ? r2.getOffhandInput() == Ingredient.EMPTY ? 0 : -1 : 1);
        recipes.forEach(recipe -> registry.add(new SpawnerDisplay(recipe)));
    }
}
